...
func Main() {
	...
	for {
		...
		case msg, ok := <-ch:
			...
			recvTime := time.Now()
			duration := recvTime.Sub(sendTime)
			recvCount++
			total += duration
			//fmt.Printf("[%s] %s -> %s msg %s send %s recv %s time %s\n", data.ClientMsgID, user.GetLoginUserID(), msg.UserID, data.TextElem.Content, sendTime.Format("2006-01-02 15:04:05.000"), recvTime.Format("2006-01-02 15:04:05.000"), duration.String())
